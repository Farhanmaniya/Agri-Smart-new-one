...
const AnalysisResults = ({ results }) => {
  ...
  // Handle simplified API response format - only prediction and confidence
  const prediction = results.prediction || 'Unknown Disease';
          ...
          <div className="flex items-start justify-between mb-4">
            ...
            <div className="flex flex-col items-end space-y-2">
              <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusBg(confidence)} ${getStatusColor(confidence)}`}>
                {getSeverityFromConfidence(confidence)} Confidence
              </span>
          ...
          <div className="p-3 bg-muted/50 rounded-lg">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className="text-muted-foreground">TensorFlow CNN</p>
              </div>
              <div>
                <span className="font-medium text-card-foreground">Classes:</span>
                <p className="text-muted-foreground">42 Disease Types</p>
              </div>
            </div>
          </div>
        </div>
