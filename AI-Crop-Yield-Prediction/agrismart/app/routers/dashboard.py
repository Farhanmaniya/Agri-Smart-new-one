...
yield_model = None
try:
    model_path = os.path.join(os.path.dirname(__file__), "..", "ml_models", "saved_models", "yield_prediction_model.joblib")
    yield_model = joblib.load(model_path)
    logger.info("Yield prediction model loaded successfully")
except Exception as e:
    logger.error(f"Failed to load yield prediction model: {str(e)}")
...
async def predict_yield_test(request: YieldPredictionRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        model_path = "ml_models/saved_models/yield_prediction_model.joblib"
        try:
            import joblib
            ...
            input_data = np.array([[
                hash(request.state) % 1000,  # Simple state encoding
                hash(request.crop) % 100,    # Simple crop encoding
                request.area,
                request.year
            ]])
            ...
            return {
                "success": True,
                "message": "Yield prediction completed using original model",
                "model_used": "Original_v1.0",
                "data": {
                    "predicted_yield_tons_per_hectare": round(prediction, 2),
                    "total_production_tons": round(prediction * request.area, 2),
                    "confidence": None,  # Original model doesn't provide confidence
                    "yield_category": "Unknown",
                    "factors_analysis": {},
                    "recommendations": [],
                    "input_parameters": {
                        "state": request.state,
        ...
        except Exception as original_model_error:
            logger.error(f"Original model failed: {str(original_model_error)}")
            raise HTTPException(
                status_code=500,
                detail=f"Both models failed. New model: {str(new_model_error) if 'new_model_error' in locals() else 'Not available'}. Original model: {str(original_model_error)}"
            )
...
async def predict_yield(
    ...
    try:
        ...
        if yield_model is None:
            raise HTTPException(
                status_code=500,
                detail="No yield prediction model available"
            )
        
        # Load the dataset to get unique values for mapping
        import pandas as pd
        df = pd.read_csv('D:/new-folder1/AI-Crop-Yield-Prediction/agrismart/app/ml_models/saved_models/India Agriculture Crop Production.csv')
        ...
        unique_seasons = df['Season'].dropna().unique().tolist()
        
        # Map categorical inputs to numerical values based on dataset
        state_mapping = {state: idx for idx, state in enumerate(unique_states)}
        district_mapping = {district: idx for idx, district in enumerate(unique_districts)}
        crop_mapping = {crop: idx for idx, crop in enumerate(unique_crops)}
        season_mapping = {season: idx for idx, season in enumerate(unique_seasons)}
